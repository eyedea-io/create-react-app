...
export function collection<T extends IAnyType>(model: T, name: string) {
  const base = types
    ...
    .views(self => ({
       ...
       */
      countBy(filter: (item: SnapshotOrInstance<T>) => any): number {
    ...
    .actions(self => ({
       ...
       * store.users.set([{id: 1, username: 'john'}])
       */
      ...
      remove(
        ...
        const removeByModelOrByNumber = (
          idOrModel: number | SnapshotOrInstance<T>
        ) => {
          if (typeof idOrModel === 'number') {
            const itemInCollection = self.items.find(
              item => item.id === idOrModel
            )
    ...
    .actions(self => ({
      ...
      load: async function<Args>(
        request: (args: Args) => Promise<SnapshotOrInstance<T>[]>,
        args: Args
      ) {
        return request(args).then(self.set)
      }
    }))
